...
public class ApiKeyCommand extends Command {
    ...
    public void handle(String[] args) {
        ...
        if (args[0].equalsIgnoreCase("check")) {
            String service = args.length > 1 ? args[1].toLowerCase() : "openai";
            
            String apiKey = ApiKeyManager.getApiKey(service);
            boolean hasKey = apiKey != null && !apiKey.isEmpty();
            
            if (hasKey) {
                String maskedKey = maskApiKey(apiKey);
                sendMessage(EnumChatFormatting.GREEN + service + " API key is set: " + maskedKey);
                
                if (service.equals("openai")) {
                    sendMessage(EnumChatFormatting.YELLOW + "The @gostats123 functionality should be working.");
                    sendMessage(EnumChatFormatting.YELLOW + "If not, try toggling it off and on with /gostats123");
                }
            } else {
                sendMessage(EnumChatFormatting.RED + service + " API key is not set!");
                
                if (service.equals("openai")) {
                    sendMessage(EnumChatFormatting.YELLOW + "The @gostats123 feature requires an OpenAI API key.");
                    sendMessage(EnumChatFormatting.YELLOW + "Use: /gmapi openai YOUR_API_KEY to set your key");
                }
            }
            return;
        }
    ...
    private void sendUsage() {
        ...
        sendMessage(EnumChatFormatting.GOLD + " - /gmapi check " + 
                     EnumChatFormatting.WHITE + "- Check API key status");
